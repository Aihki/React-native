...
const Comments = ({media}: {media: MediaItemWithOwner}) => {
  ...
  const doComment = async (inputs: {comment_text: string}) => {
    ...
    try {
      await postComment(inputs.comment_text, media.media_id, token);
      await getComments();
      reset();
    } catch (error) {
  ...
  const getComments = async () => {
    try {
      const comments = await getCommentsByMediaId(media.media_id);
        ...
        <Card>
          ...
          <Controller
            ...
            rules={{
              required: {
                value: true,
                message: 'Olkaa hyvä ja kirjoittakaa kommentti.',
              },
            }}
